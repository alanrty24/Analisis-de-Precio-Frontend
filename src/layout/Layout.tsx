...
const Layout = () => {
  const [activeSection, setActiveSection] = useState<LayoutSection>('dashboard')
  ...
  return (
    ...
    <div className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.18),transparent_30%),linear-gradient(135deg,#fffaf0_0%,#f8fafc_52%,#ecfeff_100%)] font-['Inter'] text-slate-900">
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-4 px-3 py-3 pb-28 sm:px-5 sm:py-5 sm:pb-28 lg:flex-row lg:gap-6 lg:px-8 lg:py-8 lg:pb-8">
        <aside className="hidden w-72 shrink-0 flex-col justify-between rounded-4xl border border-white/70 bg-white/70 p-5 shadow-[0_18px_80px_rgba(15,23,42,0.08)] backdrop-blur lg:fixed lg:top-8 lg:flex lg:h-[calc(100vh-4rem)]">
          <div className="space-y-8">
            <div className="space-y-3">
              ...
              <div>
                <p className="text-sm font-medium text-slate-500">The Curator</p>
            ...
            <nav className="space-y-3">
              {sectionItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className={`flex h-11 w-11 items-center justify-center rounded-xl ${isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                      <MaterialSymbol name={item.icon} className="text-[22px]" />
                    </span>
                    <span className="min-w-0">
                      <span className={`block text-sm font-semibold ${isActive ? 'text-emerald-700' : 'text-slate-900'}`}>
                        {item.label}
                      </span>
                      <span className={`block text-sm ${isActive ? 'text-emerald-600' : 'text-slate-500'}`}>
                        {item.description}
                      </span>
                    </span>
                  </button>
        ...
        <div className="flex flex-1 flex-col gap-3 sm:gap-4 lg:ml-74">
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="truncate text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500">The Curator</p>
                  <h1 className="truncate font-['Manrope'] text-lg font-extrabold text-slate-950 sm:text-xl">{activeItem.label}</h1>
                </div>
              </div>
          ...
          <main className="flex-1 overflow-x-hidden px-1 pb-2 sm:px-2 lg:px-3 lg:pb-0">{activeContent}</main>
        </div>
      </div>
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <h2 className="font-['Manrope'] text-xl font-extrabold text-slate-950">Cambiar seccion</h2>
          </div>
          <button
            type="button"
        ...
        <nav className="space-y-2 pb-1">
          {sectionItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={`flex h-9 w-9 items-center justify-center rounded-xl ${isActive ? 'bg-white text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
        ...
        <div className="mx-auto flex max-w-md items-center justify-between rounded-[1.6rem] border border-white/80 bg-white/90 px-2 py-2 shadow-[0_20px_50px_rgba(15,23,42,0.18)] backdrop-blur-xl">
          {sectionItems.map((item) => {
            ...
            return (
              ...
              >
                <MaterialSymbol name={item.icon} className={`text-[20px] ${isActive ? 'text-emerald-700' : 'text-slate-500'}`} />
                <span className={`text-[11px] font-semibold ${isActive ? 'text-emerald-700' : 'text-slate-600'}`}>
                  {item.label}
                </span>
              </button>
            )
